...
*/
void moveSnakeArray(int snakeXY[][SNAKE_ARRAY_SIZE],int testXY[][SNAKE_ARRAY_SIZE],int snakeLength, int direction)
{
	int i;
	for( i = snakeLength-1; i >= 1; i-- )
	{
		...
		testXY[0][i] = testXY[0][i-1];
		testXY[1][i] = testXY[1][i-1];
	}
...
void move(int snakeXY[][SNAKE_ARRAY_SIZE], int testXY[][SNAKE_ARRAY_SIZE],  int snakeLength, int direction)
{
	int x;
	int y;
	int x1;
	...
	x1 = testXY[0][snakeLength-1];
	y1 = testXY[1][snakeLength-1];
	//using BLANK to remove the tail
	gotoxy(x,y);
	printf("%c",BLANK);
	gotoxy(x1,y1);
	printf("%c",BLANK);
...
void startGame( int snakeXY[][SNAKE_ARRAY_SIZE], int testXY[][SNAKE_ARRAY_SIZE], int foodXY[], int consoleWidth, int consoleHeight, int snakeLength, int direction, int score, int speed)
{
	int gameOver = 0;
	clock_t endWait;
...
void loadSnake(int snakeXY[][SNAKE_ARRAY_SIZE], int testXY[][SNAKE_ARRAY_SIZE], int snakeLength)
{
	int i;
	/*
...
void prepairSnakeArray(int snakeXY[][SNAKE_ARRAY_SIZE], int testXY[][SNAKE_ARRAY_SIZE],  int snakeLength)
{
	int i, x;
	int snakeX = snakeXY[0][0];
	int snakeY = snakeXY[1][0];
	int testX = testXY[0][0];
	int testY = testXY[1][0];
...
void loadGame(void)
{
	...
	int testXY[2][SNAKE_ARRAY_SIZE]; //Two Dimension Array, the first arrauy is for the X coordinates and the second array for the Y coordinates 
	...
	testXY[0][0] = 60;
	testXY[1][0] = 10;
	loadEnviroment(consoleWidth, consoleHeight); //borders of the LED Matrix
	...
	startGame(snakeXY, testXY,foodXY, consoleWidth, consoleHeight, snakeLength, direction, score, speed);
